...
export default function MainPage() {
    ...
    const handleKeyUp = (e) => {
        ...
        if ((e.target.value || "").trim().length > 0) {
            ...
            setHideInstantLauncherIcon(false);
        }
        ...
        if (e.keyCode === 40) {
            ...
            if (skCtx.queryActive) {
                if (activeResult < skCtx.queryResults.length - 1) {
                    ...
                    <div className="cFlex col jstart acenter" style={{ height: "fit-content" }}>
                        {!authCtx.setting.minimalistic && <div className="cW100p mB30 cFlex row jcenter acenter logoContainer">
                            <svg className="logo" width="479" height="131" viewBox="0 0 479 131" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <image xlinkHref="/shortkey_logo.png" height={"130px"} />
                            </svg>

                        </div>}
                    </div>
                    <div className="cFlex col jstart acenter" style={{ position: 'absolute' }}>
                        <div className="cW100p">
                            <div
                                className={("findbar") + ((skCtx.jumpsActive && skCtx.jumpsResults.length > 0) ? " jumps-active" : "")}>
                                <div className="inner-findbar">
                                    <div>
                                        <input ref={finderRef} autoFocus={true} onBlur={handleBlur} onKeyUp={handleKeyUp} onFocus={handleFocus} />
                                    </div>
                                    <div className="autoW">
                                        {hideInstantLauncherIcon ? <button onClick={handleClearFinder}
                                            className="btnIco showOnlyOnHover">
                                            <span>
                                                <CloseIcon />
                                            </span>
                                        </button> : ''}
